...
class AlphaW_Node_mk(Gamma_Unobserved_Variational_Node):
    ...
    def updateParameters(self):
        ...
        ESs, EWWs = [tmp_es["ES"] for tmp_es in tmp], [tmp_eww["EWW"] for tmp_eww in tmp]
        ...
        # Perform updates
        Qa = Pa + 0.5*np.sum([ES.shape[0] for ES in ESs])
        Qb = Pb + 0.5*np.sum([EWW.sum(axis=0) for EWW in EWWs], axis=0)
